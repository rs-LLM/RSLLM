//! 价格规则服务模块
//! 提供价格规则的管理和计算功能
use crate::domain::dto::price_rule::{
    CreatePriceRuleDTO, PriceCalculationDTO, PriceRuleQueryDTO, UpdatePriceRuleDTO,
};
use crate::domain::table::ai_hub::price_rule::AiHubPriceRule;
use crate::domain::vo::price_rule::{
    AiHubPriceRuleVO, AppliedRuleVO, PriceCalculationVO, PriceRuleOverviewVO,
};
...
impl PriceRuleService {
    ...
    pub async fn update_rule(&self, id: &str, dto: UpdatePriceRuleDTO) -> ApplicationResult<()> {
        ...
        if let Some(effective_start) = &dto.effective_start {
            rule.effective_start = Some(DateTime::from_str(effective_start).map_err(|e| {
        ...
        if let Some(effective_end) = &dto.effective_end {
            rule.effective_end = Some(DateTime::from_str(effective_end).map_err(|e| {
                ApplicationError::ValidationError {
                    message: format!("Invalid effective_end: {}", e),
                    field: Some("effective_end".to_string()),
    ...
    pub async fn list_rules(
        &self,
        query: PriceRuleQueryDTO,
    ) -> ApplicationResult<Vec<AiHubPriceRuleVO>> {
        ...
        for rule in all_rules {
            ...
            if let Some(rule_name) = &query.rule_name
                && !rule.rule_name.contains(rule_name)
            {
                continue;
            }
            ...
            if let Some(true) = query.active_only {
                ...
                let effective_start = rule.effective_start.clone().unwrap_or(
                    DateTime::from_str("1970-01-01T00:00:00").map_err(|_| {
        ...
        if let Some(page) = query.page
            && let Some(page_size) = query.page_size
        {
            let page = page.max(1);
    ...
    pub async fn get_overview(&self) -> ApplicationResult<PriceRuleOverviewVO> {
        ...
        let all_active_rules =
            AiHubPriceRule::select_by_map(pool!(), rbs::value!("status": "active")).await?;
        ...
        for rule in all_active_rules {
            let effective_start =
                rule.effective_start
                    .unwrap_or(DateTime::from_str("1970-01-01T00:00:00").map_err(|_| {
                        ApplicationError::ConfigError {
                            message: "Invalid default date".to_string(),
                            key: Some("default_date".to_string()),
                        }
    ...
    pub async fn calculate_price(
        &self,
        dto: PriceCalculationDTO,
    ) -> ApplicationResult<PriceCalculationVO> {
    ...
    async fn get_matching_rules(
        &self,
        dto: &PriceCalculationDTO,
    ) -> ApplicationResult<Vec<AiHubPriceRule>> {
        let now = DateTime::now();
    ...
    fn rule_matches(&self, rule: &AiHubPriceRule, dto: &PriceCalculationDTO) -> bool {
        ...
        if let Some(user_level) = &dto.user_level
            && let Some(conditions_obj) = conditions.as_object()
            && let Some(required_level) = conditions_obj.get("user_level")
            && let Some(level_str) = required_level.as_str()
            && level_str != user_level
        {
            return false;
        }
            ...
            && let Some(conditions_obj) = conditions.as_object()
        {
            if let Some(min_usage) = conditions_obj.get("min_usage")
                && let Some(min) = min_usage.as_f64()
                && total_usage < &min
            {
                return false;
            }
            if let Some(max_usage) = conditions_obj.get("max_usage")
                && let Some(max) = max_usage.as_f64()
                && total_usage > &max
            {
                return false;
            }
        ...
        if let Some(conditions_obj) = conditions.as_object() {
            if let Some(min_tokens) = conditions_obj.get("min_tokens")
                && let Some(min) = min_tokens.as_i64()
                && (dto.input_tokens + dto.output_tokens) < min
            {
                return false;
            }
            if let Some(max_tokens) = conditions_obj.get("max_tokens")
                && let Some(max) = max_tokens.as_i64()
                && (dto.input_tokens + dto.output_tokens) > max
            {
                return false;
            }
        }
