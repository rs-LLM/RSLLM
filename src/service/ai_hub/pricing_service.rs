...
// 用途：导入计费标准DTO
// 说明：用于接收请求数据
use crate::domain::dto::ai_hub::{CreatePricingDTO, QueryPricingDTO, UpdatePricingDTO};
// 用途：导入计费标准VO
// 说明：用于返回响应数据
use crate::domain::vo::ai_hub::PricingVO;
// 用途：导入模型基础表
// 说明：用于查询模型信息
use crate::domain::table::ai_hub::ModelBase;
// 用途：导入数据库连接池
// 说明：用于获取数据库连接
use crate::pool;
...
impl PricingService {
    ...
    pub async fn get_pricing(&self, id: &str) -> ApplicationResult<PricingVO> {
        ...
        let model_name = if let Ok(models) =
            ModelBase::select_by_map(pool!(), rbs::value! { "id": &pricing.model_id }).await
        {
            if !models.is_empty() {
    ...
    /// 根据模型ID查询计费标准
    pub async fn get_pricing_by_model(
        &self,
        model_id: &str,
    ) -> ApplicationResult<Option<PricingVO>> {
        let pricings =
            Pricing::select_by_map(pool!(), rbs::value! { "model_id": model_id }).await?;
