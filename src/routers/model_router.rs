...
impl ModelRouter {
    ...
    pub async fn route_to_model_with_fallback(
        ...
        for mapping in mappings {
            match self
                ...
                .await
            {
                Ok(routing_info) => return Ok(routing_info),
                Err(e) => {
                    log::warn!(
    ...
    pub async fn route(
        ...
        let provider = self.route_to_provider(model_identifier).await?;
    ...
    pub async fn list_all_models_page(
        ...
        let end = (offset + page_size as usize).min(all_model_infos.len());
